...
async function checkTables() {
    try {
        ...
        const { rows: [ statics ] } = await client.query(`
            SELECT EXISTS (
                SELECT FROM 
                    pg_tables
                WHERE 
        ...
        tablesExist.statics = statics.exists;
        ...
        tablesExist.players = players.exists;
        
        const { rows: [ gearsets ] } = await client.query(`
            SELECT EXISTS (
                SELECT FROM 
                    pg_tables
                WHERE 
        ...
        tablesExist.gearsets = gearsets.exists;
        ...
        tablesExist.users = users.exists;
...
async function createTestUsers() {
    if (!tablesExist.users) {
        await createUser('jaaski', 'readytosail')
    }
}

async function createTestStatic() {
    if (!tablesExist.statics) {
...
async function createTestPlayers() {
    ...
    if (!tablesExist.players) {
        await createPlayer("lime grove", "malboro", 6, 'whm', 1);
        await createPlayer("top text", "malboro", 12, 'drk', 1);
        await createPlayer("gurr aarg", "malboro", 16, 'gnb', 1);
        await createPlayer("rawr wars", "malboro", 9, 'sch', 1);
        await createPlayer("ambrosia eternum", "malboro", 10, 'nin', 1);
        await createPlayer("genevieve frilaix", "malboro", 18, 'rpr', 1);
        await createPlayer("aava sihn", "malboro", 8, 'smn', 1);
        await createPlayer("ratboy rights", "malboro", 17, 'dnc', 1);
    }
    console.log('done creating test players')
}
